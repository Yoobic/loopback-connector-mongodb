  ...
  Object.keys(where).forEach(function(k) {
    ...
    if (spec) {
      if (spec === 'between') {
        ...
        query[k] = { $regex: new RegExp(escapeRegex(cond), options) };
      } else if (spec === 'nlike') {
        query[k] = { $not: new RegExp(escapeRegex(cond), options) };
      } else if (spec === 'neq') {
        query[k] = { $ne: cond };
      } else if (spec === 'regexp') {
        if (cond.global)
...
MongoDB.prototype.all = function all(model, filter, options, callback) {
  ...
  function processResponse(err, cursor) {
    ...
    if (!hasNearFilter(filter.where)) {
      ...
      if (process.env.ENABLE_MONGO_CONNECTOR_CASEINSENSITIVE_SORT === 'true') {
        var lbModelProperties = _.get(options, 'method.ctor.definition.properties');
        if (lbModelProperties) {
          var caseInsensitiveProperties = _.chain(lbModelProperties).pickBy(value => {
            return value.caseInsensitive;
          }).keys().value()
          if (_.intersection(caseInsensitiveProperties, _.keys(order)).length > 0) {
...
MongoDB.prototype.count = function count(model, where, options, callback) {
  ...
  this.execute(model, 'count', where, countOptions, function(err, count) {
    if (self.debug) {
