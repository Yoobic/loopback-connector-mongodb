...
function get_MAX_TIME_MS () {
  if (global.yoobicConfigHelper) {
    return global.yoobicConfigHelper.get_MAX_TIME_MS();
  } else {
    return parseInt(process.env.MAX_TIME_MS) || 33 * 1000;
  }
}

function mirrorRequest ({ collectionName, op, query: params, sort, skip, limit, options, ctx }) {
  if (global.yoobicMirrorHelper) {
    return global.yoobicMirrorHelper.mirror({ collectionName, op, query: params, sort, skip, limit, options, ctx });
  }
}
...
MongoDB.prototype.create = function(model, data, options, callback) {
  ...
  const createOptions = { safe: true };
  if (options.bypassDocumentValidation === true){
    createOptions.bypassDocumentValidation = options.bypassDocumentValidation;
  }
...
MongoDB.prototype.save = function(model, data, options, callback) {
  ...
  this.execute(model, 'replaceOne', {_id: oid}, data, {upsert: true}, function(err, result) {
    if (!err) {
...
MongoDB.prototype.replaceOrCreate = function(model, data, options, cb) {
  ...
  const replaceOrCreateOptions = { upsert: true };
  if (options.bypassDocumentValidation === true){
    replaceOrCreateOptions.bypassDocumentValidation = options.bypassDocumentValidation;
  }
  this.replaceWithOptions(model, oid, data, replaceOrCreateOptions, cb);
};
    ...
    if (spec) {
      ...
      if (spec === 'between') {
        ...
        if (cond instanceof RegExp) {
          ...
          query[k] = {$regex: new RegExp(escapeRegex(cond), regexOptions)};
        }
      } else if (spec === 'nlike') {
        if (cond instanceof RegExp) {
          query[k] = {$not: cond};
        } else {
          query[k] = {$not: new RegExp(escapeRegex(cond), regexOptions)};
        }
      } else if (spec === 'neq') {
        query[k] = {$ne: cond};
      } else if (spec === 'regexp') {
        ...
        query[k] = {$regex: escapeRegex(cond)};
      } else {
        query[k] = {};
        query[k]['$' + spec] = cond;
      }
...
MongoDB.prototype.all = function all(model, filter, options, callback) {
  ...
  var findOpts = { maxTimeMS: get_MAX_TIME_MS() };
  if (fields) {
    findOpts.projection = fieldsArrayToObj(fields);
  }
  if (hint) {
    findOpts.hint = hint;
  }
  ...
  function processResponse(err, cursor) {
    ...
    mirrorRequest ({ collectionName: model, op: 'find', query, sort: order, skip: filter.skip, limit: filter.limit, options: findOpts });
...
MongoDB.prototype.count = function count(model, where, options, callback) {
  ...
  where = self.buildWhere(model, where, options) || {};
  let skipOrLimitInOptions = options && (options.skip || options.limit);
  const method = Object.keys(where).length === 0 && !skipOrLimitInOptions ? 'estimatedDocumentCount' : 'countDocuments';
  ...
  if (method === 'estimatedDocumentCount') {
    ...
    var countOptions = { maxTimeMS: get_MAX_TIME_MS() };
    if (options) {
      if (options.skip) {
      ...
      if (options.hint) {
        countOptions.hint = options.hint;
      }
    }
    mirrorRequest ({ collectionName: model, op: 'countDocuments', query: where, options: countOptions });
    this.execute(model, method, where, countOptions, function(err, count) {
      if (self.debug) {
        debug('count.callback', model, err, count);
      }
      if (callback) {
